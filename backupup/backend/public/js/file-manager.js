    ...
    categories.forEach(cat => {
      ...
      card.innerHTML = `
        ...
        <div class="upload-area" id="upload-${cat}">
          <input type="file" id="file-input-${cat}" multiple class="file-input-hidden" data-category="${cat}" accept="${fileExtensions[cat] && fileExtensions[cat].length > 0 ? fileExtensions[cat].join(',') : '*'}">
...
function renderUserList(users) {
  ...
  tbody.innerHTML = users.map(u => {
    const logoHtml = u.logo_url 
      ? `<img src="/uploads/${u.logo_url}" alt="Logo" class="user-logo-preview" data-user-id="${u.id}">
         <div class="user-logo-placeholder logo-error-hidden">🏢</div>`
      : `<div class="user-logo-placeholder">🏢</div>`;
    
    const logoActions = u.logo_url
      ? `<div class="logo-actions">
           <button class="btn-logo btn-logo-upload" data-action="upload-logo" data-user-id="${u.id}" data-user-name="${u.name || 'İsimsiz'}" title="Logo Değiştir">📤</button>
           <button class="btn-logo btn-logo-delete" data-action="delete-logo" data-user-id="${u.id}" title="Logo Sil">🗑️</button>
         </div>`
      : `<button class="btn-logo btn-logo-upload" data-action="upload-logo" data-user-id="${u.id}" data-user-name="${u.name || 'İsimsiz'}" title="Logo Yükle">📤</button>`;
    ...
    <tr>
      ...
      <td>${u.phone}</td>
      <td>${u.role || 'user'}</td>
      <td><span class="${u.is_active ? 'text-success' : 'text-danger'}">${u.is_active ? 'Aktif' : 'Pasif'}</span></td>
      <td>${u.assignedProjects && u.assignedProjects.length>0 ? u.assignedProjects.map(p=>`<span class="project-tag">${p.project_name||p.project_code||`Proje ${p.id}`}</span>`).join(' ') : '<span class="no-projects-text">Atanmamış</span>'}</td>
      <td>
        <div class="user-actions">
          ${logoActions}
          <button class="edit-user-btn" data-user-id="${u.id}">Düzenle</button>
          <button class="delete-user-btn" data-user-id="${u.id}">Sil</button>
        </div>
      </td>
    </tr>
    `;
  ...
  // Logo error handling
  setTimeout(() => {
    document.querySelectorAll('.user-logo-preview').forEach(img => {
      img.addEventListener('error', function() {
        this.classList.add('logo-error-hidden');
        const placeholder = this.nextElementSibling;
        if (placeholder) placeholder.classList.add('logo-error-visible');
      });
    });
  }, 100);
}
...
async function editUser(userId) {
  try {
    ...
    modal.innerHTML = `
      <div class="modal-content">
        ...
        <form id="editUserForm">
          ...
          <label style="display:block;margin-bottom:5px;margin-top:10px;">İsim:</label>
          <input name="name" value="${user.name||''}" />
          <label style="display:block;margin-bottom:5px;margin-top:10px;">Yeni Şifre (opsiyonel):</label>
          <input type="password" name="password" placeholder="Yeni şifre (boş bırakılırsa değişmez)" />
          
          <label style="display:block;margin-bottom:5px;margin-top:15px;">Proje Atamaları:</label>
          <input type="text" id="project-search" class="project-search" placeholder="Proje ara..." />
          <div class="project-options" id="project-options" style="max-height:200px;overflow:auto;"></div>
          <div class="selected-projects" id="selected-projects"></div>
          
          <label style="display:block;margin-bottom:5px;margin-top:15px;">Rol:</label>
          <select name="role">
          ...
          <label style="display:block;margin-bottom:5px;margin-top:10px;">Durum:</label>
          <select name="is_active">
            <option value="true" ${user.is_active?'selected':''}>Aktif</option>
            <option value="false" ${!user.is_active?'selected':''}>Pasif</option>
          </select>
          <div class="modal-buttons">
            <button type="button" class="modal-cancel-btn">İptal</button>
    ...
    let selectedProjectIds = [...assignedIds];
    
    function renderProjectOptions(filteredProjects = projects) {
      const container = document.getElementById('project-options');
      container.innerHTML = filteredProjects.map(p => `
        <div class="project-option ${selectedProjectIds.includes(p.id) ? 'selected' : ''}" data-project-id="${p.id}">
          <strong>${p.project_code || p.id}</strong> - ${p.project_name || p.name || `Proje ${p.id}`}
        </div>
      `).join('');
    }
    ...
    function renderSelectedProjects() {
      ...
      container.innerHTML = selected.map(p => `
        <span class="selected-project-tag">
          ${p.project_code || p.project_name || p.name || `Proje ${p.id}`}
          <span class="remove-project" data-project-id="${p.id}">×</span>
        </span>
      `).join('');
    }
    ...
    // Project selection
    document.getElementById('project-options').addEventListener('click', (e) => {
    ...
    document.getElementById('selected-projects').addEventListener('click', (e) => {
...
let currentLogoUserId = null;

function openLogoModal(userId, userName) {
  currentLogoUserId = userId;
...
function closeLogoModal() {
  ...
  currentLogoUserId = null;
}

async function uploadUserLogo() {
  const fileInput = document.getElementById('logo-file');
  const file = fileInput?.files[0];
  
  if (!file || !currentLogoUserId) {
  ...
  try {
    ...
    const response = await fetch(`/admin/users/${currentLogoUserId}/logo`, {
      method: 'POST',
      credentials: 'include',
      body: formData
    });
    ...
    showMessage('Logo yüklenirken hata oluştu: ' + error.message, 'error');
  } finally {
    if (uploadBtn) {
      uploadBtn.disabled = false;
      uploadBtn.innerHTML = '<i class="fas fa-upload mr-2"></i>Yükle';
    }
  }
}

async function deleteUserLogo(userId) {
  if (!confirm('Bu kullanıcının logosunu silmek istediğinizden emin misiniz?')) {
    return;
  }

  try {
    const response = await fetch(`/admin/users/${userId}/logo`, {
      method: 'DELETE',
      credentials: 'include'
    });
    ...
    showMessage('Logo başarıyla silindi!', 'success');
    loadUsers();

  } catch (error) {
    console.error('Error deleting logo:', error);
    showMessage('Logo silinirken hata oluştu: ' + error.message, 'error');
  }
}
...
function setupEventListeners() {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (e.target.dataset.action === 'upload-logo') {
      const userId = e.target.dataset.userId;
      const userName = e.target.dataset.userName;
      openLogoModal(userId, userName);
    }
    if (e.target.dataset.action === 'delete-logo') {
      const userId = e.target.dataset.userId;
      deleteUserLogo(userId);
    }
  });
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Logo modal event listeners
  const cancelUploadBtn = document.getElementById('cancel-upload');
  if (cancelUploadBtn) cancelUploadBtn.addEventListener('click', closeLogoModal);
  
  const uploadLogoBtn = document.getElementById('upload-logo');
  if (uploadLogoBtn) uploadLogoBtn.addEventListener('click', uploadUserLogo);
