...
const parsedCors = parseCorsOrigins(process.env.CORS_ORIGIN || '');

const config = {
  port: process.env.PORT || 3001,
  sessionSecret: process.env.SESSION_SECRET || 'dev_secret_change_me',
  // In production we require explicit allowlist (enforced in index.js)
  corsOrigin: parsedCors.length > 0 ? parsedCors : [],
