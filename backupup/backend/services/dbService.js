...
class UserService {
  ...
  static getUserByPhone(phone) {
    return new Promise((resolve, reject) => {
      db.get(
        'SELECT id, phone, password_hash, is_active, name, role, created_at, last_login, logo_url FROM users WHERE phone = ?',
        [phone],
        (err, row) => {
          if (err) {
  ...
  static getAllUsers() {
    return new Promise((resolve, reject) => {
      db.all(
        'SELECT id, phone, name, role, is_active, created_at, last_login, logo_url FROM users ORDER BY created_at DESC',
        [],
        (err, rows) => {
          if (err) {
            reject(handleDatabaseError(err));
  ...
  static getUserById(userId) {
    return new Promise((resolve, reject) => {
      db.get(
        'SELECT id, phone, password_hash, is_active, name, role, created_at, last_login, logo_url FROM users WHERE id = ?',
        [userId],
        (err, row) => {
          if (err) {
            reject(handleDatabaseError(err));
   ...
   * Update user logo
   * @param {number} userId - User ID
   * @param {string|null} logoUrl - Logo URL or null to remove
   * @returns {Promise<boolean>} Success status
   */
  static updateUserLogo(userId, logoUrl) {
    return new Promise((resolve, reject) => {
      db.run(
  ...
  static deleteProject(projectId) {
    return new Promise((resolve, reject) => {
      db.serialize(() => {
        ...
        // Clean up related records first (order matters due to foreign key constraints)
              ...
              db.run(
                ...
                function(err) {
                  if (err) {
                    ...
                    db.run('COMMIT');
                    console.log(`🗑️ Project ${projectId} permanently deleted from database along with all related records`);
                    console.log(`   - Deleted from: user_projects, project_assets, project_settings, model_versions, gallery_images, projects`);
                    resolve(this.changes > 0);
                  }
                }
              );
