...
export default function AdminViewer({ projectId = 1 }) {
  ...
  viewer.scene.globe.depthTestAgainstTerrain = true;
    ...
    // Wait for a valid projectCode before loading tileset
    const code = (projectCode && projectCode.trim()) ? projectCode.trim() : null;
    if (!code) {
      console.warn('[AdminViewer] projectCode not ready; skipping tileset load');
      return () => {
        disposed = true;
    ...
    const base = `/${code}_project`;
    const projectTilesetUrl = `${base}/tiles/sezyum_${code}.json`;
  ...
  window.__ADMIN_TILESET = tileset;
      ...
      fetch(createApiUrl(`api/projects/${projectId}/model/latest`), {
        ...
        .then(response => {
          ...
          if (!modelData) {
            // No saved data for this project yet: provide a default transform so tools can place the model
      ...
      console.error('Admin tileset load error', err);
