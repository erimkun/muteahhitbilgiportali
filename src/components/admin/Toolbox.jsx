...
export default function Toolbox() {
  const { activeTool, setActiveTool, measurement, selection, areaMode, setAreaMode, setClipping, boxTool, setBoxTool, viewer, tileset, buildingEntity, buildingTransform, setBuildingTransform, logoTransform } = useCesiumCtx();
  ...
  const fetchHistory = React.useCallback(() => {
    fetch('http://localhost:3001/api/model/history')
      .then(res => res.json())
      .then(response => {
        if (response.data) {
  ...
  async function publishState() {
    ...
    try {
      ...
      const saveResponse = await fetch('http://localhost:3001/api/model', {
        method: 'POST',
      ...
      const publishResponse = await fetch(`http://localhost:3001/api/model/publish/${newVersionId}`, {
        method: 'PUT',
      });
  ...
  async function revertTo(id) {
    try {
      const publishResponse = await fetch(`http://localhost:3001/api/model/publish/${id}`, {
        method: 'PUT',
      });
      if (!publishResponse.ok) throw new Error('Failed to publish model version.');

      // Fetch the data for the reverted version to apply it to the runtime
      const modelResponse = await fetch(`http://localhost:3001/api/model/published`);
